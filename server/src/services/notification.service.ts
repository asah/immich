...
export class NotificationService extends BaseService {
  ...
  async onActivityCreate({ activityId, actorId, albumId, assetId, parentActivityId }: ArgOf<'ActivityCreate'>) {
  ...
  async onAssetDescriptionUpdate({ assetId, actorId }: ArgOf<'AssetDescriptionUpdate'>) {
    ...
    await this.jobRepository.queue({ name: JobName.NotifyAssetDescription, data: { id: assetId, recipientId: asset.ownerId, actorId } });
  }
  ...
  async handleAssetDescription({ id, recipientId, actorId }: JobOf<JobName.NotifyAssetDescription>) {
    ...
    const item = await this.notificationRepository.create({ userId: recipientId, type: NotificationType.AssetUpdate, level: NotificationLevel.Info, title: 'Photo description updated', description: `${actor.name} updated a photo description`, data: JSON.stringify({ assetId: id }) });
    this.websocketRepository.clientSend('on_notification', recipientId, mapNotification(item));
    const { server } = await this.getConfig({ withCache: false });
    const { html, text } = await this.emailRepository.renderEmail({ template: EmailTemplate.ACTIVITY, data: { baseUrl: getExternalDomain(server), albumId: id, albumName: 'a photo', actorName: actor.name, recipientName: recipient.name, activity: 'updated the description of', itemPath: `/photos/${id}` }, customTemplate: '' });
