...
<main class="min-h-dvh bg-immich-bg px-4 pt-8 pb-28 text-immich-fg dark:bg-immich-dark-bg dark:text-immich-dark-fg sm:px-8">
  <div class="mx-auto max-w-5xl">
    ...
    {#if session}
      <p class="mt-4 text-sm font-medium">{session.submittedCount} rated · rate {Math.max(0, session.requiredCount - session.submittedCount)} more to unlock the full results</p>
        ...
        {#each session.candidates as candidate (candidate.assetId)}
          <article class="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-immich-dark-gray">
            <div
              ...
              style:aspect-ratio={candidate.width && candidate.height ? `${candidate.width} / ${candidate.height}` : undefined}
            >
              <img
                class="max-h-[70dvh] w-full object-contain"
                src={getAssetMediaUrl({ id: candidate.assetId, size: AssetMediaSize.Original })}
                alt="Rate this"
                loading="lazy"
              />
              {#if isWorthZooming(candidate.width, candidate.height)}
                <button
                  ...
                  title="Inspect detail at full size"
                ><Icon icon={mdiMagnifyPlus} size="16" /> Zoom for detail</button>
              {/if}
            </div>
            <div class="flex items-center gap-2 p-3">
              <Button size="small" color={candidate.value === 1 ? 'primary' : 'secondary'} disabled={busyAssetId === candidate.assetId} onclick={() => vote(candidate.assetId, candidate.value === 1 ? null : 1)}>👍 Vote</Button>
              <Button size="small" color="secondary" class={candidate.value === -1 ? 'text-red-600' : ''} disabled={busyAssetId === candidate.assetId} onclick={() => vote(candidate.assetId, candidate.value === -1 ? null : -1)}>👎 Not for me</Button>
              {#if candidate.value}
                <button class="ms-auto text-sm text-gray-500 hover:underline" onclick={() => vote(candidate.assetId, null)}>Clear</button>
              {/if}
            </div>
          </article>
        {/each}
      ...
      {#if session.leaderboardVisible}
        ...
        <div class="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/95 shadow-lg backdrop-blur dark:border-gray-700 dark:bg-immich-dark-bg/95">
          <div class="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3 sm:px-8">
            <Button color="secondary" disabled={remainingRatings > 0} onclick={openLeaderboard}>
              {remainingRatings > 0 ? `Rate ${remainingRatings} more to reveal community favorites` : 'See community favorites'}
            </Button>
            <a class="text-sm font-medium text-primary hover:underline" href={backHref}>Back to album</a>
          </div>
        </div>
        ...
        {#each leaderboard.items.slice(0, 12) as item, index (item.assetId)}
          <li>
            <div class="grid h-36 place-items-center rounded-lg bg-black">
              <img class="h-full w-full object-contain" src={getAssetMediaUrl({ id: item.assetId })} alt={`Favorite ${index + 1}`} />
            </div>
